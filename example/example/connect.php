...
if (isset($_POST['write_file'])) {
    if(isset($_SESSION['server']) && isset($_SESSION['database']) && isset($_SESSION['username']) && isset($_SESSION['password'])) {
        ...
        if ($result->num_rows > 0) {
            $file_name = isset($_POST['filename']) ? $_POST['filename'] : 'course_list.txt';
            $file_content = '';

            while ($row = $result->fetch_assoc()) {
                $file_content .= "ID: " . $row['id'] . "\nTitle: " . $row['title'] . "\nDescription: " . $row['description'] . "\nImage URL: " . $row['imageUrl'] . "\n\n";
            }
...
// Update database name to db_lethiphuongthanh
$_SESSION['database'] = 'db_lethiphuongthanh';
?>
